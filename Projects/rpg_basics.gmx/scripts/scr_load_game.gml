///scr_load_game
var file = file_text_open_read(working_directory+"save_file.txt");
var save_string = file_text_read_string(file);//get entire file as a string
file_text_close(file);

//decode the file
save_string = base64_decode(save_string);//turn json into data struct
var save_data = json_decode(save_string);

var save_room = save_data[? "room"];//ds_map...? = accessor so access the room from the data struct
if(room != save_room){
    room_goto(save_room);
}

with(obj_player_stats){//load in the player by setting coordinates
